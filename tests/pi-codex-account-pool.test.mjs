...
test("quota displays official usage windows without token-budget inference", () => {
  ...
  assert.equal(compactQuota(quota, 1_699_999_001_000), "20% · 3h 4m", "compact line uses the tightest window");
...
test("compact quota shows time until the binding reset, not the window duration", () => {
  ...
  for (const [seconds, expected] of [[583200, "6d 18h"], [187200, "2d 4h"], [3660, "1h 1m"], [60, "1m"], [1, "1m"], [0, "0m"], [-60, "0m"], [undefined, "?"]]) {
    assert.equal(compactQuota(quota(seconds), now), `38% · ${expected}`);
  }
  assert.equal(compactQuota(quota(187200), now + 3600000), "38% · 2d 3h");
  assert.equal(compactQuota(undefined, now), "?% · ?");
});
...
test("headroom is the tightest window across every limit; limit-reached is zero", () => {
  ...
  assert.equal(compactQuota({ fetchedAt: 1_700_000_000_000, windows: [{ limitId: "codex", limitReached: true, primary: { usedPercent: 100, resetsAt: 1_700_000_000 } }] }, 1_700_000_000_000), "0% · 0m");
});
...
test("bare pi-ai stream() calls from other extensions route through the pool while it is enabled", { timeout: 30000 }, async t => {
  const { root, path } = await fixture(t);
  const agent = join(root, "agent"), cwd = join(root, "project"), output = join(root, "bare.json"), probe = join(root, "bare.ts");
  const jwt = Buffer.from(JSON.stringify({ "https://api.openai.com/auth": { chatgpt_account_id: "fixture" } })).toString("base64url");
  await mkdir(cwd, { recursive: true }); await mkdir(agent, { recursive: true }); await writeFile(join(agent, "settings.json"), "{}");
  await updatePoolState(state => { state.enabled = true; state.accounts.push({ ...account("one", "one", 4_102_444_800_000), access: `x.${jwt}.x` }); }, path);
  // Mirrors pi-condense's summarizer: pi-ai's compat stream(), not the registry, with the stock login as apiKey.
  await writeFile(probe, `import {writeFileSync} from 'node:fs'; import {stream} from '@earendil-works/pi-ai/compat'; export default function(pi){pi.registerCommand('bare',{handler:async(_a,ctx)=>{const m=ctx.modelRegistry.find('openai-codex','gpt-6-luna');const auths=[];for await(const _ of stream(m,{messages:[{role:'user',content:[{type:'text',text:'hi'}],timestamp:1}]},{apiKey:'stock-login-token',fetch:async(_u,init)=>{auths.push(new Headers(init.headers).get('authorization'));return new Response(JSON.stringify({error:{code:'rate_limit_exceeded'}}),{status:429});}})){};writeFileSync(${JSON.stringify(output)},JSON.stringify(auths));}})}`);
  const run = exec(process.execPath, [cli, "--offline", "--no-session", "--no-context-files", "--approve", "-e", join(checkout, "pi/extensions/codex-account-pool"), "-e", probe, "-p", "/bare"], { cwd, env: { ...process.env, HOME: root, PI_CODING_AGENT_DIR: agent }, timeout: 25000 }); run.child.stdin.end(); await run;
  assert.deepEqual(JSON.parse(await readFile(output, "utf8")), [`Bearer x.${jwt}.x`], "the pool account token is used, not the caller's stock login");
});
