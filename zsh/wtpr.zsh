...
# Pulls my open PRs from the work repo with `gh`, then opens a new iTerm window
# with a tab per PR and runs `wt <branch>` in each. `wt` (defined in zshrc) cd's
# to that branch's worktree, creating it via work.py if it doesn't exist yet.
#
# usage: wtpr [repo-path] [-n|--dry-run]
#   repo-path     repo to read PRs from (default: ~/dev/work)
