...
_work_auto_activate_venv() {
  local git_root
  git_root="$(git rev-parse --show-toplevel 2>/dev/null)" || return

  local REPO="$HOME/dev/work"

  # Already in the correct venv? Skip.
  if [[ -n "$VIRTUAL_ENV" ]]; then
    if [[ "$git_root" == "$REPO" && "$VIRTUAL_ENV" == *"/work" ]]; then
      return
  ...
  export PYTHONPATH="$git_root:$_WORK_BASE_PYTHONPATH"
}

autoload -U add-zsh-hook
add-zsh-hook chpwd _work_auto_activate_venv
_work_auto_activate_venv  # Run once at startup for shells opened inside a worktree
...
wt () {
  ...
  local output
  output="$("$root/scripts/work.py" create worktree "$branch" "$@")" || return 1
...
cwt () {
  ...
  if [[ -n "$VIRTUAL_ENV" && "$VIRTUAL_ENV" != *"/work" ]]; then
    deactivate
  fi
