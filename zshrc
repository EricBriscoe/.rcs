...
export REPO=$HOME/dev/work
export _WORK_BASE_PYTHONPATH="$PYTHONPATH"
export PYTHONPATH=$REPO:$_WORK_BASE_PYTHONPATH
...
# the git root ($root/venv), then $WORKON_HOME/<main-repo-name>; the name is derived from
# git-common-dir, so every worktree of work shares ~/.venvs/work. Only ever
...
_auto_venv() {
  ...
  # A venv we didn't activate is the user's business, so leave it alone
...
work() {
  local script
  script="$(git rev-parse --show-toplevel 2>/dev/null)/scripts/work.py"
  if [[ ! -x "$script" ]]; then
...
alias lsql='docker exec -it app_db psql -U app -d app'
...
wt () {
  ...
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || root="$HOME/dev/work"
  ...
  # work.py runs under `python`, which lives in the repo's virtualenv. The chpwd
  ...
  output="$(
    ...
    "$root/scripts/work.py" create worktree "$branch" "$@"
  )" || return 1

  # work.py prints "cd <path>" on the last line; extract the path
  local target
...
cwt () {
  ...
  "$root/scripts/work.py" clean worktree "$@" || return 1
...
work_wt_up () {
  ...
  if [[ ! -x "$root/scripts/work.py" ]]; then
    echo "error: $root/scripts/work.py not found"
    return 1
  fi
  ...
  "$root/scripts/work.py" tilt down || return 1

  echo "Installing dependencies and building local packages..."
  "$root/scripts/work.py" ci || return 1

  echo "Starting full stack with Tilt..."
  "$root/scripts/work.py" tilt up --all
}
export PATH="$HOME/.local/bin:$PATH"
export PATH="$HOME/.cargo/bin:$PATH"

# WORK personal glossary: `lodef` opens it, `lodef <term>` greps for a term.
# Renders with glow if available; falls back to plain text.
lodef() {
  local glossary="$HOME/Documents/WORK/glossary.md"
  if [[ ! -f "$glossary" ]]; then
...
lodef-edit() { ${EDITOR:-code} "$HOME/Documents/WORK/glossary.md"; }
