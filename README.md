...
Pi setup installs Node ≥22.19, ripgrep, fd, Pi, Playwright/Chromium, checksum-verified RTK, and packages from `pi/settings.json`. Conflicts are backed up; `--skip-install` reconciles settings and relinks resources. Both setup modes maintain a marked block in Git’s configured global ignore file (otherwise the existing XDG Git ignore file, or `~/.gitignore`). It ignores `specs/`, `.bigpowers/`, `allure-results/`, and `.pi/{subagents,npm,git,mcp-traces,mcp-oauth}/` at any depth. Existing rules and tracked files are preserved; agent instructions and project configuration are not ignored.
