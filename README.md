...
- installs `virtualenvwrapper` and creates the `work` venv
...
- auto-activates the `work` venv at startup, and per-worktree `venv/` when `cd`-ing into one (`chpwd` hook)
- adds `$HOME/.local/bin` and the `work` repo to `PYTHONPATH`
- `compinit` cached to once per day
- sources `fnm`, `zoxide`, `fzf`, iTerm2 integration, Docker completions when present

Functions and aliases:

- `work <args>`: runs `scripts/work.py` from the current repo, falling back to `~/dev/work`
...
- `work_wt_up`: `tilt down` → `work ci` → `tilt up --all`
- `lsql`: psql into the local WORK Postgres container
...
- LSPs prefer WORK-local binaries when present: `~/dev/work/node_modules/.bin/biome`, `~/.venvs/work/bin/{python,ruff}`. Falls back to `$PATH` otherwise.
