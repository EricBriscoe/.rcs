...
4. If empty, report nothing to commit. Otherwise make one descriptive commit. Honor checks/hooks; stop on failure except `pi-maintenance`'s Markdown-budget repair.
5. Report hash, summary, and remaining changes. Do not repeatedly commit newly arriving edits.

No push, branch switching, amend, stash/discard, bypassed hooks, unsolicited implementation or unrelated check-fixing edits.
