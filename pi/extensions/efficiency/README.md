...
External usage: pi-condense's summarizer calls never become assistant messages, so transcript readers (token-widget) cannot see them. pi-condense publishes cumulative cost on Pi's extension event bus (`cost:external`); each flush becomes an `external-usage` session entry with the summarizer model (from `contextPrune.summarizerModel`, else the session model) and input/output tokens. Cache lanes are not reported by pi-condense and are recorded as zero, so that spend prices as uncached input. Custom entries never enter the model context.
