...
State: agent-directory `tooling/memory-embedding/<pin-hash>/`, outside Git. Model: 23,685,172 bytes (22,972,370 weights). macOS arm64: 50 dependency tarballs, 128,824,758 bytes; ~153 MB combined download, ~416 MB installed (~406 MiB allocated). No privileged install. New pins require setup/restart; helpers support `/reload`.

Tests: `node --test tests/pi-memory*.test.mjs`. Offline model tests/evaluation: set `PI_MEMORY_EMBEDDING_AGENT` to a setup-completed agent directory; stores are synthetic. `pi-memory-retrieval-eval.test.mjs`: 33 notes/19 queries, FTS→hybrid recall@8 9/13→11/13, MRR .654→.769; identifiers top-1 3/3, unrelated false hits 0/6. Warm ~2–3ms, cold model-ready .18–1.42s, first query ~5ms, cold FTS ~2ms here; clean setup ~9s (npm cache warm). Two paraphrases miss; lexical distractors remain; negation ≠ equivalence. The coarse .40 gate is not calibrated confidence; this fixture is not a held-out guarantee. `PI_MEMORY_LIVE=1` separately enables authenticated extraction.
