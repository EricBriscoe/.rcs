...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "monitor",
    label: "Monitor",
    description: "Run background Bash commands with explicit notifyOn for start: output wakes Pi on batched stdout/stderr and exit status; completion buffers output and wakes Pi only once the process exits and its output streams close. List jobs, read new output manually, or stop a job. Automatic delivery waits until Pi settles. Commands keep running between turns. Each job retains the latest 16000 characters; dropped output is reported. Up to eight jobs are retained; all stop when the Pi session closes or changes (including reload).",
